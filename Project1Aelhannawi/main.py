...
def get_data_and_put_in_db(db_cursor: sqlite3.Cursor):
    ...
    CreateDataBase.put_top_250_in_database("top_show_data", top_show_data_for_db, db_cursor)
    CreateDataBase.put_top_250_in_database("top_movie_data", top_movie_data_for_db, db_cursor)
    CreateDataBase.put_most_popular_in_database("most_popular_movies", most_pop_movies, db_cursor)
    CreateDataBase.put_most_popular_in_database("most_popular_shows", most_pop_tv, db_cursor)
    CreateDataBase.put_in_wheel_of_time(db_cursor)
    ...
    CreateDataBase.put_ratings_into_db(db_ready_ratings_data, db_cursor)
    CreateDataBase.put_ratings_into_db(big_mover_ratings, db_cursor)

def main():
    connection, db_cursor = CreateDataBase.open_db("project1db.sqlite")
    CreateDataBase.create_all_tables(db_cursor)
    get_data_and_put_in_db(db_cursor)
    CreateDataBase.close_db(connection)
