...
def get_top_250_data(what_kind: str) -> list[dict]:
    api_query = f"https://imdb-api.com/en/API/Top250{what_kind}s/k_8rtu00s8"
...
def get_most_popular(type: str) -> list[tuple]:
    api_query = f"https://imdb-api.com/en/API/MostPopular{type}/k_8rtu00s8"
    response = requests.get(api_query)
    if response.status_code != 200:  # if we don't get an ok response we have trouble
        print(f"Failed to get data, response code:{response.status_code} and error message: {response.reason} ")
        sys.exit(-1)
...
def get_big_mover_ratings(big_movers: list[tuple]) -> list[tuple]:
    ...
    for mover in big_movers:
        url = f"https://imdb-api.com/en/API/UserRatings/k_8rtu00s8/{mover[0]}"  # the ttid is in position 0
